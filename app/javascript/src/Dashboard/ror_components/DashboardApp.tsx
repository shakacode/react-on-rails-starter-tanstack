...
function RootLayout() {
  ...
  return (
    <main className="tanstack-shell">
      ...
      {showDevtools ? <TanStackRouterDevtools router={router} position="bottom-right" /> : null}
    </main>
  );
}
...
function ProjectsTable({
  ...
  // REFERENCE PATTERN: tanstack-table — see AGENTS.md §7
  const { api } = useDashboardProps();
...
const installRouterStoreShim = (router: ReturnType<TanStackRouterOptions['createRouter']>) => {
  type MutableRouter = ReturnType<TanStackRouterOptions['createRouter']> & {
    __store?: {
  ...
  const mutableRouter = router as MutableRouter;
