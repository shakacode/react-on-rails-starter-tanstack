...
const developmentEnvOnly = (clientWebpackConfig, _serverWebpackConfig, _rscWebpackConfig) => {
  ...
  // Enable React Refresh (Fast Refresh) only when the normalized dev server is
  // in HMR mode. Requires @pmmmwh/react-refresh-webpack-plugin and the
  // react-refresh/babel plugin (already wired in babel.config.js behind
  // WEBPACK_SERVE). Skipped for static watch builds.
  if (clientWebpackConfig.devServer?.hot && process.env.WEBPACK_SERVE) {
