...
This starter begins from `create-react-on-rails-app --rsc --rspack` and currently targets the React on Rails Pro `17.1.0-rc.5` RC stack with React on Rails RSC `19.3.0-rc.4` and Shakapacker `10.2.0`.

Shakapacker remains on `10.2.0` because the React on Rails 17.1 RC5 upgrade does not require a coupled Shakapacker bump. Rspack is the checked-in local and deploy default. `react-on-rails-rsc@19.3.0-rc.4` provides the Rspack RSC manifest support this starter needs, while Webpack remains an opt-in bridge/comparison path.
...
with `react-on-rails-rsc@19.3.0-rc.4`. The Rspack client, server, and
