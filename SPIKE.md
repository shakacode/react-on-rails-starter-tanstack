...
The app was bootstrapped with `create-react-on-rails-app --rsc --rspack --package-manager pnpm` and the current release stack targets React on Rails Pro `17.0.0`, React on Rails RSC `19.2.1`, and Shakapacker `10.2.0`. Shakapacker stays on `10.2.0` because the React on Rails 17 final upgrade did not require a coupled Shakapacker bump. The Rails/Rspack/React on Rails Pro setup passes `react_on_rails:doctor` with the expected warning that both Rspack and Webpack configs are present.
...
- React on Rails and React on Rails Pro release stack: `17.0.0`.
...
- Keep the React on Rails Pro `17.0.0`, React on Rails RSC `19.2.1`, and Shakapacker `10.2.0` release stack.
